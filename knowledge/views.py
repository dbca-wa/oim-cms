...
def itsystemreq(request):
    ...
    return render(request, 'itsystemform.html', {'PermCheck': PermsResp})


def ITSystemObj(self):
    # Get It system Request Data and Custdian Basic Info and return a json
    # response.
    ...
    for value in resp2:
        ...
        if value.system_creation_date:
            itsobj['reqsystem_creation_date'] = value.system_creation_date.strftime(
                '%d/%m/%Y')
        else:
            itsobj['reqsystem_creation_date'] = ""

    if itsobj['reqcustodianid']:
        DepartmentUser.objects.only(
            'id', 'given_name', 'surname', 'title', 'org_unit')
        ...
        for value in rows:
            ...
            itsobj['labelfield'] = value.given_name + \
                ' ' + value.surname + ' - ' + value.title
            itsobj['searchfield'] = value.given_name + \
                ' ' + value.surname + ' ' + value.title
...
def PeopleObj(self):
    # Allow filtered list of people based on name and job title keyword search
    ...
    if keyword:
        rows = DepartmentUser.objects.filter(
            name__contains=keyword) | DepartmentUser.objects.filter(
...
def SaveITSystemRequest(self):
    ...
    itsystemreq.system_creation_date = datetime.strptime(
        reqsystem_creation_date, '%d/%m/%Y')
...
def ITSystemPermssionCheck(self, reqid):
    ...
    if permObj['groupcheck']:
        return True
    elif permObj['custodiancheck']:
        return True
    elif permObj['superUserCheck']:
        return True
    else:
        return False
