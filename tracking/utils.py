...
def migrate_data_hardware_computer():
    """Single-use migration script to copy data from Hardware to Computer
    models, prior to removal.
    """
    from registers.models import Hardware, ITSystemHardware, Backup
    from .models import Computer

    for h in Hardware.objects.all():
        # Ensure that a Computer exists for each Hardware.
        if not h.computer:
            if not Computer.objects.filter(hostname=h.name).exists():
                # We need to create a Computer.
                c = Computer(
                    hostname=h.name, manufacturer='Unknown', model='Unknown',
                    chassis='Unknown', serial_number='Unknown', os_name='Unknown',
                    os_version='Unknown', os_service_pack='Unknown',
                    os_arch='Unknown', cpu='Unknown')
                c.save()
                h.computer = c
            else:
                h.computer = Computer.objects.filter(hostname=h.name).first()
            h.save()

        # Preserve the location field info.
        if h.computer and h.location:
            c = h.computer
            c.location = h.location
            c.save()

        # For ITSystemHardware, swap Hardware for Computer obj.
        for i in ITSystemHardware.objects.filter(host=h):
            i.computer = c
            i.save()

    # Ensure that all Backup objects have been updated.
    for b in Backup.objects.all():
        h = b.system
        if not h.computer:  # We need to create a Computer.
            c = Computer(
                hostname=h.name, manufacturer='Unknown', model='Unknown',
                chassis='Unknown', serial_number='Unknown', os_name='Unknown',
                os_version='Unknown', os_service_pack='Unknown',
